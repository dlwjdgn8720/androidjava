...
public class ProductDAO {
	...
	public List<ProductVO> titlesearch(String ptitle) throws Exception { // 파라미터 = 매개변수
		...
		// <>안에는 컨테이너에 무엇을 넣을 지정!
		// 3. sql문을 만든다.
		String sql = "select * from product where product_title like '" + ptitle + "%'";
 ...
 // 상품명으로 상품 아이디 검색
   ...
   //관리자 상품검색
	...
	public boolean productupdate(ProductVO bag) throws Exception {
		con = dbcp.getConnection();
        String sql = "update product set product_name= ?, product_title = ?, "
				      + "product_explanation = ?, product_price= ?, product_inventory= ? "
				      + "where product_id= ?";
		ps = con.prepareStatement(sql);
		ps.setString(1, bag.getProduct_name());
